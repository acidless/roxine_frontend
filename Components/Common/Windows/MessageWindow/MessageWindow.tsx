...
const MessageWindow: React.FC<IMessageWindow> = function ({isScrollable=false, onKeyDown, window, children, isOpened, toggleOpen }) {
  ...
  function toggleWindow(e: React.MouseEvent<HTMLDivElement | HTMLButtonElement>) {
    if (e.target === currentTarget && currentTarget === windowBackground.current) {
      toggleOpen(!isOpened);
    }
  }
  ...
  return (
    <div
      ...
      className={`message-window-wrapper content-center flex-container${isOpened ? " active" : ""}${isScrollable ? " scrollable" : ""}`}
    >
